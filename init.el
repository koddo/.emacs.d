...
(defun my/scroll-command (n-lines)
  (interactive)
  (let ((scrolling-p (or (eq last-command #'my/scroll-down-command)
                         (eq last-command #'my/scroll-up-command))))
    (unless
        scrolling-p
    ...
    (let* ((next-screen-line (+ my/scroll-command---virtual-cur-line n-lines)))
      ...
      (when (and (<= (line-number-at-pos (window-start)) (line-number-at-pos my/scroll-command---point-before-scrolling))
                 (<= (line-number-at-pos my/scroll-command---point-before-scrolling) (- (line-number-at-pos (window-end nil t)) 0)))
        (progn
          ...
          ;; (let ((n-lines-from-top (- my/scroll-command---virtual-cur-line (line-number-at-pos (window-start)))))
          ;;   (when (< n-lines-from-top (window-body-height))
          ;;     (setq my/scroll-command---n-lines-from-top n-lines-from-top))
          ;;   )
          (setq my/scroll-command---n-lines-from-top (- my/scroll-command---virtual-cur-line (line-number-at-pos (window-start))))
          )))))
